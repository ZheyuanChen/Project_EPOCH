...
def parse_input_deck(parent_dir, sdf_dir):
    ...
    if not block_text:
        raise ValueError(
            "Could not find 'name = xy_energy' dist_fn block in input.deck."
        )

    # Extract range1 (Angle)
    range1_match = re.search(
        r"range1\s*=\s*\(\s*([^,]+)\s*,\s*([^)]+)\s*\)", block_text
    )
    if not range1_match:
        raise ValueError("Could not find 'range1 = (...)' in the xy_energy block.")

    # Extract range2 (Energy in Joules)
    range2_match = re.search(
        r"range2\s*=\s*\(\s*([0-9\.eE\+\-]+)\s*,\s*([0-9\.eE\+\-]+)\s*\)", block_text
    )
    if not range2_match:
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot Angle-Energy Heatmaps directly from SDF files."
    )
    parser.add_argument(
        "--dir",
        dest="input_dir",
        required=True,
        help="Input directory containing the .sdf files",
    )
    parser.add_argument(
        "--out", type=str, default=None, help="Output directory for heatmaps"
    )
    ...
    try:
        emin_mev, emax_mev, angle_min_deg, angle_max_deg, res1, res2 = parse_input_deck(
    ...
    for sdf_file in sdf_files:
        ...
        if plotted_anything:
            fig.suptitle(
                f"Angle_Energy Heatmap at t = {sim_time_fs:.1f} fs", fontsize=16
            )
            plt.tight_layout()
            out_path = os.path.join(
                output_dir, f"xy_angle_energy_heatmap_{file_num}.png"
            )
