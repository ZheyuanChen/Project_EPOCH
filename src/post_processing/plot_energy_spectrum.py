...
def parse_input_deck(parent_dir, sdf_dir):
    ...
    block_pattern = re.compile(
        r"begin:dist_fn.*?name\s*=\s*spatial_energy.*?end:dist_fn",
    ...
    if not match:
        raise ValueError(
            "Could not find 'name = spatial_energy' dist_fn block in input.deck."
        )
    ...
    range_match = re.search(
        r"range3\s*=\s*\(\s*([0-9\.eE\+\-]+)\s*,\s*([0-9\.eE\+\-]+)\s*\)", block_text
    )
    if not range_match:
        raise ValueError(
            "Could not find 'range3 = (min, max)' in the spatial_energy block."
        )
    ...
    if not res_match:
        raise ValueError(
            "Could not find 'resolution3 = ...' in the spatial_energy block."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot 1D energy spectra from 3D EPOCH SDF distributions."
    )
    parser.add_argument(
        "--dir",
        dest="input_dir",
        type=str,
        help="Directory containing the .sdf files (e.g., .../parent_dir/sdf_files)",
    )
    parser.add_argument(
        "--out",
        type=str,
        default=None,
        help="Output directory for plots. Defaults to parent_dir/energy_spectrum",
    )
    parser.add_argument(
        "--emin", type=float, default=None, help="Minimum energy in GeV"
    )
    parser.add_argument(
        "--emax", type=float, default=None, help="Maximum energy in GeV"
    )
    parser.add_argument(
        "--res", type=int, default=None, help="Energy resolution (number of bins)"
    )
    ...
    if None in (emin, emax, res):
        print(
            "Missing range or resolution arguments. Falling back to input.deck parsing..."
        )
    ...
    print(
        f"Plotting Parameters -> Range: {emin:.3e} to {emax:.3e} GeV | Resolution: {res} bins"
    )
    ...
    for sdf_file in sdf_files:
        ...
        if hasattr(data, "dist_fn_spatial_energy_Electron"):
            ...
            plt.plot(
                en_grid_ev, spec_elec, label="Electrons", color="blue", linewidth=2
            )
